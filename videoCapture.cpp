...
int VideoCapture::set_io_method(enum io_method io_meth){
        switch (io_meth)
        {
                case IO_METHOD_READ:
                case IO_METHOD_MMAP:
...
int VideoCapture::stop_capturing(void)
{
...
int VideoCapture::start_capturing(void)
{
        unsigned int i;
        enum v4l2_buf_type type;
...
int VideoCapture::uninit_device(void)
{
        unsigned int i;
        int ret = 0;
...
int VideoCapture::init_read(unsigned int buffer_size)
{
...
int VideoCapture::init_mmap(void)
{
        struct v4l2_requestbuffers req;
        int ret = 0;
...
int VideoCapture::init_userp(unsigned int buffer_size)
{
        struct v4l2_requestbuffers req;
...
int VideoCapture::init_device(unsigned int width, unsigned int height, unsigned int format)
{
...
int VideoCapture::close_device(void)
{
        if (-1 == close(fd))
        {
...
int VideoCapture::open_device(const char *dev_name)
{
        struct stat st;
...
int VideoCapture::init_cam(int index, unsigned int width, unsigned int height, unsigned int format, enum io_method io_meth)
{
	char devname[20];
...
int VideoCapture::get_cam_frame(unsigned char **pointer_to_cam_data, int *size)
{
        if (!is_initialised)
        {
...
int VideoCapture::release_cam_frame()
{
        if (!is_initialised)
        {
        ...
        if (-1 == ioctl(fd, VIDIOC_QBUF, &frame_buf))
        {
